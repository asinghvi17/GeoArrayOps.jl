# # Grid interface proof of concept — v2
#
# This example implements the grid interface described in
# `docs/src/grid-interface-decisions.md`:
#
# - Each grid stores a manifold for geometry dispatch; rectilinear grids also
#   store their original CRS for reconstructing Raster outputs.
# - Out-of-bounds neighbors are removed by index. Kernels handle nodata values
#   that occur inside the grid.
# - `neighborgeometry` computes reusable distances and bearings once per call.
# - `cellarea(grid)` returns all cell areas, while `cellarea(grid, I)` returns
#   the area of one cell.
# - `splitspatial` separates grid-construction keywords from algorithm
#   keywords, and public methods restore Raster metadata on their outputs.
# - `mapneighbors` passes neighbor records, `mapwindow` passes a named north-up
#   window, and `eachneighbor` supports explicit traversal algorithms.
# - Horn slope estimation requires a rectilinear window. Plane fitting uses
#   neighbor records and therefore supports both grid types.
...
#
# `neighborgeometry(grid, rings)` precomputes distances and bearings at the
# coarsest level where they remain constant: one table for a planar grid, one
# table per latitude row for a spherical rectilinear grid, or one centroid per
# DGG cell. This avoids recomputing geometry inside stencil kernels.
#
...
#
# Queue-driven algorithms such as priority flood, accumulation, and HAND can
# precompute geometry and reuse it while visiting cells:
#
#     geom = neighborgeometry(grid, NeighborRings(1))
#     visit!(out, geom, ras, queue) # a function barrier; see `_accumulate!`
#
# Both grid types return lazy records with the fields `index`, `value`,
...
cellarea(grid::CellGrid) =
    [_cellareaof(grid.levelgrid, c, manifold(grid).radius) for c in grid.cells]
...
function mapneighbors(f, ras, grid::RectilinearGrid, rings::NeighborRings=NeighborRings())
    ...
    valid = CartesianIndices(ras)
    # The stencil, stored offsets, and geometry table use the same logical
    # offset order, so each zipped value, index, and geometry record describes
    # the same neighbor.  The `in valid` filter relies on
    # `Stencils.indices` leaving out-of-bounds indices unclamped under `Remove`.
...
# DGG performs cell traversal and infers the output type. Neighbor records reuse
# the centroid cache created for `eachneighbor`.
...
# ## Algorithms
#
# Steepest slope and flow direction inspect neighbor records independently of
# the underlying grid type.
...
function _flow_direction(_, value, neighbors)
    ...
    return bearing # Clockwise from north; NaN indicates a pit or edge outlet
end
...
# This D8-style accumulation processes cells from highest to lowest and sends
# each cell's accumulated area to its steepest downhill neighbor. The same
# traversal works for both grid types.
...
function flowaccumulation(ras, grid::AbstractGridSpec)
    ...
    return _accumulate!(acc, ras, grid, geom, cells)
end

# `neighborgeometry`'s concrete type is only known at runtime, so traversal
# loops must sit behind a function barrier like this one to specialize on it;
# inlining this loop above costs ~35x in time and allocations.
function _accumulate!(acc, ras, grid, geom, cells)
    for c in cells
...
# Flow accumulation uses explicit `eachneighbor` traversal and conserves the
# total input area.
acc = flowaccumulation(raster_xy)
@assert Rasters.name(acc) == :flowaccumulation
outlets = isnan.(parent(direction_xy))
@assert sum(parent(acc)[outlets]) ≈ sum(cellarea(grid_xy)) # All area reaches an outlet
@assert maximum(parent(acc)) ≈ length(xs) * 10.0 # Each row drains east into one outlet
...
# All spherical cell area reaches an outlet.
@assert sum(parent(cell_acc)[cell_outlets]) ≈ sum(cell_areas)
